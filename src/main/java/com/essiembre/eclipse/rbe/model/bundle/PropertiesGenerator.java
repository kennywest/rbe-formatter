...
public final class PropertiesGenerator {
     ...
     */
    public static <T extends Appender> T generate(Bundle bundle, T appender) {
        ...
        if (headComment != null && headComment.length() > 0) {
            if (RBEPreferences.getShowGenerator()
                ...
                appender.append(GENERATED_BY);
                appender.append(SYSTEM_LINE_SEP);
            }
            appender.append(headComment);
        }
...
//        text.append(key);
    ...
    private static String getKeyGroup(String key) {
        ...
        return getKeyGroup(key, deepness);
    }

    private static String getKeyGroup(String key, int deepness) {
        ...
        if (levelFound != 0) {
            if (levelFound < deepness) {
                if (deepness != RBEPreferences.getMinGroupLevelDeepness()) {
                    return getKeyGroup(key, RBEPreferences.getMinGroupLevelDeepness());
                } else {
                    return key;
                }
            }
